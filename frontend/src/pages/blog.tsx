...
const Blog: React.FC<{data: Array<{title: string, description: string, content: string, imgUrl: string, _id: string}>}> = ({ data }) => {
    ...
    return(
        <Fragment>
            <div className="w-full overflow-scroll flex flex-col items-center h-screen px-8 space-y-8 static h-screen bg-neutral-950">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        {
                            state.posts.length > 0 && state.posts.map((post: {title: string, description: string, content: string, imgUrl: string, _id: string}, index: number) => (
                                <div onClick={() => router.push(`/blog/${post._id}`)} className={`px-4 py-4 hover:cursor-pointer bg-neutral-950 hover:bg-neutral-900 h-fit rounded rounded-2xl flex flex-col transition ${index === 0 ? 'lg:col-span-2 lg:max-h-[800px]' : 'max-w-xl'}`}>
